# Token 详解：什么是 Admin Token？

## 🎫 Token 就像"门票"或"房卡"

### 生活类比

想象你去酒店：
1. **办理入住**时，前台给你一张**房卡** → 这就是 Token
2. **使用服务**时，你需要**刷房卡** → 这就是 API 调用时带上 Token
3. **前台验证**房卡是否有效 → 网关验证 Token

```
┌─────────────┐                    ┌─────────────┐
│  客户端应用   │  ←── Token卡 ──→  │  网关服务器   │
│ (你的程序)   │   "我有权限！"     │ (Gateway)   │
└─────────────┘                    └─────────────┘
```

---

## 🔑 在 Gateway Zero 中，Token 的作用

### 1. 身份识别
```
客户端请求：
  "我想调用 GPT-4"
  
网关检查：
  "你是谁？" → 看 Token
  "Token: my-test-token-12345"
  
网关查数据库：
  "这个 Token 存在吗？" → ✅ 存在
  "这个 Token 启用了吗？" → ✅ enabled = true
  "这个 Token 有额度吗？" → ✅ 没超限
  
网关放行：
  "好的，帮你调用 GPT-4"
```

### 2. 权限控制

一个 Token 可以设置很多限制：

```sql
-- 数据库中的 Token 记录
token                     | my-test-token-12345
allowed_models            | gpt-4,claude-3  (只能用这两个模型)
max_amount                | 100.0           (最多花 100 元)
enabled                   | 1               (启用状态)
expires_at                | 2025-12-31      (过期时间)
amount_spent              | 25.5            (已花费 25.5 元)
```

### 3. 用量统计

网关会记录每个 Token 的使用情况：
- 用了多少 tokens（输入 + 输出）
- 花了多少钱
- 调用了哪些模型
- 什么时候调用的

---

## 🛠️ 为什么第一个 Token 要手动创建？

### 问题：鸡生蛋，蛋生鸡

```
场景1：通过 API 创建 Token
┌──────────────────────────────────────┐
│ POST /admin/tokens                   │
│ Authorization: Bearer ???            │  ← 需要一个 Token 才能创建 Token！
│ { "token": "new-token" }             │
└──────────────────────────────────────┘

这就形成了循环：
创建 Token 需要 Token → 但我还没有 Token → 无法创建 Token 😵
```

### 解决方案：直接在数据库创建

```
绕过 API，直接操作数据库：
┌──────────────────────────────────────┐
│ sqlite3 data/gateway.db              │
│ INSERT INTO admin_tokens ...         │  ← 直接写入数据库
└──────────────────────────────────────┘

现在有了第一个 Token → 可以用它创建更多 Token ✅
```

---

## 📝 Token 的完整生命周期

### 阶段 1：创建 Token

**方式 A：手动创建（第一个）**
```bash
sqlite3 data/gateway.db
INSERT INTO admin_tokens (name, token, ...) VALUES ('my-first-token', 'my-token', ...);
```

**方式 B：通过 API 创建（有了第一个 Token 后）**
```bash
curl -X POST http://localhost:8080/admin/tokens \
  -H "Authorization: Bearer my-first-token" \
  -d '{"max_amount": 50}'
```

### 阶段 2：使用 Token

```bash
# 每次调用 API 都需要带上 Token
curl http://localhost:8080/v1/chat/completions \
  -H "Authorization: Bearer my-test-token-12345" \
  -d '{"model": "gpt-4", "messages": [...]}'
```

**网关的验证流程**：
```rust
// 伪代码
1. 从 HTTP Header 中提取 Token
   let token = extract_token("Authorization: Bearer my-test-token-12345");
   // token = "my-test-token-12345"

2. 从数据库查询 Token
   let admin_token = db.get_token(token).await?;
   
3. 验证 Token
   if !admin_token.enabled {
       return Error("Token 已禁用");
   }
   if admin_token.expires_at < now() {
       return Error("Token 已过期");
   }
   if admin_token.amount_spent >= admin_token.max_amount {
       return Error("Token 额度已用完");
   }
   
4. 验证通过，处理请求
   let response = call_ai_provider(...);
   
5. 更新统计
   db.add_usage_spent(token, prompt_tokens, completion_tokens);
```

### 阶段 3：查看 Token 使用情况

```bash
# 查看自己的用量
curl http://localhost:8080/v1/token/usage \
  -H "Authorization: Bearer my-test-token-12345"

# 输出
{
  "total_tokens": 1250,
  "prompt_tokens": 800,
  "completion_tokens": 450,
  "amount_spent": 0.025
}
```

### 阶段 4：管理 Token

```bash
# 禁用 Token
curl -X POST http://localhost:8080/admin/tokens/<token-id>/toggle \
  -H "Authorization: Bearer admin-token"

# 删除 Token
curl -X DELETE http://localhost:8080/admin/tokens/<token-id> \
  -H "Authorization: Bearer admin-token"
```

---

## 🔐 Token 的安全性

### Token 的形式
```
my-test-token-12345  ← 这是明文，类似密码
```

### 存储位置
- **数据库**：明文存储（生产环境应加密）
- **客户端**：配置文件或环境变量
- **传输**：HTTP Header（应使用 HTTPS）

### 安全建议

1. **使用 HTTPS**
   ```
   ❌ http://gateway.com  (Token 明文传输，容易被截获)
   ✅ https://gateway.com (加密传输)
   ```

2. **定期轮换 Token**
   ```bash
   # 每个月创建新 Token，删除旧的
   curl -X POST /admin/tokens -d '{"expires_at": "2025-01-31"}'
   ```

3. **设置额度限制**
   ```bash
   # 防止单个 Token 被盗用后损失过大
   curl -X POST /admin/tokens -d '{"max_amount": 100.0}'
   ```

4. **监控异常**
   ```bash
   # 定期检查 Token 使用情况
   curl /admin/logs/requests | grep "my-token"
   ```

---

## 💡 实际使用场景

### 场景 1：团队协作

```
团队成员 A：Token-A (额度 $50，只能用 gpt-3.5)
团队成员 B：Token-B (额度 $100，可以用 gpt-4)
管理员：    Token-Admin (无限额度，所有模型)
```

### 场景 2：不同应用

```
Web 应用：   Token-Web
移动应用：   Token-Mobile
测试环境：   Token-Test (额度很小，防止浪费)
```

### 场景 3：客户计费

```
客户 1：Token-Customer1 (预充值 $100)
客户 2：Token-Customer2 (预充值 $50)
  ↓ 用完自动停用
  ↓ 需要续费
```

---

## 🎯 总结

### Token 是什么？
- 一个字符串，用于身份认证
- 存储在数据库中
- 关联权限、额度、统计信息

### 为什么需要 Token？
- **安全**：不直接暴露 AI Provider 的 API Key
- **管理**：统一管理多个用户/应用
- **统计**：追踪每个用户的使用情况
- **限额**：防止超支

### Token vs AI Provider API Key

```
┌────────────────────────────────────────────────────┐
│  客户端                                              │
│    ↓ 使用 Admin Token (my-test-token-12345)       │
│  网关                                                │
│    ↓ 使用 Provider API Key (sk-proj-xxx)          │
│  AI Provider (OpenAI/Anthropic)                    │
└────────────────────────────────────────────────────┘

好处：
✅ 客户端不需要知道真实的 AI API Key
✅ 可以切换 Provider，客户端无感知
✅ 可以对单个 Token 限额，而不是整个 API Key
```

---

## 🚀 下一步

理解了 Token 后，你可以：
1. 继续 QUICK_START.md 的步骤 3-7
2. 尝试创建多个 Token，设置不同的权限
3. 查看 Token 的使用统计
4. 阅读源码：`src/server/handlers/auth.rs`

希望这个解释清楚了！有任何问题随时问我 😊

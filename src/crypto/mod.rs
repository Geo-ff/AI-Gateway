...
fn from_hex(s: &str) -> AppResult<Vec<u8>> {
    if s.len() % 2 != 0 { return Err(GatewayError::Config("Invalid hex length".into())); }
    let mut out = Vec::with_capacity(s.len()/2);
    let bytes = s.as_bytes();
    for i in (0..s.len()).step_by(2) {
        let hi = (bytes[i] as char).to_digit(16).ok_or_else(|| GatewayError::Config("Invalid hex".into()))?;
        let lo = (bytes[i+1] as char).to_digit(16).ok_or_else(|| GatewayError::Config("Invalid hex".into()))?;
...
pub fn unprotect(strategy: &Option<KeyLogStrategy>, provider: &str, data: &str, encrypted: bool) -> AppResult<String> {
    if !encrypted { return Ok(data.to_string()); }
    match strategy.clone().unwrap_or(KeyLogStrategy::Masked) {
        ...
        KeyLogStrategy::None | KeyLogStrategy::Masked => {
            ...
            Ok(String::from_utf8(plain).map_err(|e| GatewayError::Config(format!("Invalid UTF-8 after decrypt: {}", e)))?)
        }
    }
}

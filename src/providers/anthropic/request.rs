...
pub fn convert_openai_to_anthropic(openai_req: &ChatCompletionRequest) -> anthropic::CreateMessageParams {
    ...
    let tool_choice = match openai_req.tool_choice.clone() {
        Some(oai::ChatCompletionToolChoiceOption::Named(named)) => {
            Some(anthropic::ToolChoice::Tool { name: named.function.name })
        }
            ...
            oai::ChatCompletionRequestMessage::User(m) => {
                let content = match &m.content {
                    oai::ChatCompletionRequestUserMessageContent::Text(text) => {
                        anthropic::MessageContent::Text { content: text.clone() }
                    }
                        ...
                        let blocks = parts
                            ...
                            .filter_map(|p| match p {
                                oai::ChatCompletionRequestUserMessageContentPart::Text(t) => {
                                    Some(anthropic::ContentBlock::Text { text: t.text.clone() })
                                }
            ...
            oai::ChatCompletionRequestMessage::Tool(m) => {
                ...
                let content_str = match &m.content {
                    ...
                    oai::ChatCompletionRequestToolMessageContent::Array(parts) => parts
                        .iter()
                        .map(|p| match p { oai::ChatCompletionRequestToolMessageContentPart::Text(t) => t.text.clone() })
                        .collect::<Vec<_>>()
                        .join("\n"),
                };
                blocks.push(anthropic::ContentBlock::ToolResult { tool_use_id: id, content: content_str });
    ...
    anthropic::CreateMessageParams {
        ...
        tools,
        tool_choice,
        max_tokens: openai_req.max_completion_tokens.or(openai_req.max_tokens).unwrap_or(1024) as u32,

...
pub fn adapt_openai_request_for_zhipu(mut req: oai::CreateChatCompletionRequest) -> oai::CreateChatCompletionRequest {
    // 处理 top_p
        ...
        for (i, c) in arr.iter().enumerate() {
            ...
            let tool_calls = msg.get("tool_calls").and_then(|tc| tc.as_array()).map(|arr| {
                arr.iter().filter_map(|t| {
                    ...
                    let arguments = f.get("arguments").map(|x| match x {

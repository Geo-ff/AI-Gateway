...
impl DatabaseLogger {
    ...
    pub async fn get_model_price(
        ...
        provider: &str,
        model: &str,
    ) -> Result<Option<(f64, f64, Option<String>, Option<String>)>> {
        let conn = self.connection.lock().await;
    ...
    pub async fn list_model_prices(
        &self,
        provider: Option<&str>,
    ) -> Result<Vec<(String, String, f64, f64, Option<String>, Option<String>)>> {
        let conn = self.connection.lock().await;
        if let Some(p) = provider {
            let mut stmt = conn.prepare("SELECT provider, model, prompt_price_per_million, completion_price_per_million, currency, model_type FROM model_prices WHERE provider = ?1 ORDER BY model")?;
            ...
            let mut stmt = conn.prepare("SELECT provider, model, prompt_price_per_million, completion_price_per_million, currency, model_type FROM model_prices ORDER BY provider, model")?;

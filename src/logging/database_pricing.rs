...
impl DatabaseLogger {
    ...
    pub async fn get_model_price(&self, provider: &str, model: &str) -> Result<Option<(f64, f64, Option<String>)>> {
        let conn = self.connection.lock().await;
        ...
        let row = stmt.query_row((provider, model), |row| {
            Ok((row.get::<_, f64>(0)?, row.get::<_, f64>(1)?, row.get::<_, Option<String>>(2)?))
        }).optional()?;
        Ok(row)
    }

    pub async fn list_model_prices(&self, provider: Option<&str>) -> Result<Vec<(String, String, f64, f64, Option<String>)>> {
        let conn = self.connection.lock().await;

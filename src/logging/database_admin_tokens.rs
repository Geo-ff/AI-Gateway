...
impl TokenStore for DatabaseLogger {
    async fn create_token(&self, payload: CreateTokenPayload) -> Result<AdminToken, GatewayError> {
        ...
        Ok(AdminToken {
            ...
            expires_at: match expires_at_s { Some(s) => Some(parse_beijing_string(&s)?), None => None },
            created_at: now,
            amount_spent: 0.0,
    ...
    async fn update_token(&self, token: &str, payload: UpdateTokenPayload) -> Result<Option<AdminToken>, GatewayError> {
        ...
        let row_opt = stmt.query_row([token], |row| {
            Ok((
                row.get::<_, String>(0)?,
        ...
        let Some((tok, allowed, max, enabled_i, expires, created_at_s, max_amount0, amount_spent0, prompt0, completion0, total0)) = row_opt else { return Ok(None); };
    ...
    async fn get_token(&self, token: &str) -> Result<Option<AdminToken>, GatewayError> {
        ...
        let row = stmt.query_row([token], |row| {
            ...
            Ok((token, allowed, max_tokens, enabled_i, expires, created_at_s, max_amount, amount_spent, prompt_tokens_spent, completion_tokens_spent, total_tokens_spent))
        }).optional()?;
        if let Some((token, allowed, max_tokens, enabled_i, expires, created_at_s, max_amount, amount_spent, prompt_tokens_spent, completion_tokens_spent, total_tokens_spent)) = row {
            Ok(Some(AdminToken {
                token,
                ...
                max_amount,
                enabled: enabled_i != 0,
                expires_at: match expires { Some(s) => Some(parse_beijing_string(&s)?), None => None },
                created_at: parse_beijing_string(&created_at_s)?,
    ...
    async fn list_tokens(&self) -> Result<Vec<AdminToken>, GatewayError> {
        ...
        let rows = stmt.query_map([], |row| {
            ...
            let out = AdminToken {
                ...
                max_amount,
                enabled: enabled_i != 0,
                expires_at: match expires { Some(s) => parse_beijing_string(&s).ok() , None => None },

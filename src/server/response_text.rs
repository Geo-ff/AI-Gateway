...
fn normalize_output_text(text: String) -> Option<String> {
...
fn join_text_fragments<I>(fragments: I) -> Option<String>
where
    I: IntoIterator<Item = String>,
{
    ...
    normalize_output_text(joined)
}

fn join_text_blocks<I>(blocks: I) -> Option<String>
where
    I: IntoIterator<Item = String>,
{
    let mut joined = String::new();
...
fn collect_text(value: &Value) -> Option<String> {
    match value {
        ...
        Value::Array(items) => join_text_fragments(items.iter().filter_map(collect_text)),
        Value::Object(map) => {
            for key in [
                "delta",
                "output_text",
                ...
                if let Some(text) = map.get(key).and_then(collect_text) {
                    return Some(text);
                }
            }
            None
...
fn extract_from_choices(raw: &Value) -> Option<String> {
    ...
    if let Some(content) = message.and_then(|message| message.get("content"))
        && let Some(text) = collect_text(content)
    {
        return Some(text);
    }
...
fn extract_from_output(raw: &Value) -> Option<String> {
    if let Some(text) = raw.get("output_text").and_then(collect_text) {
        return Some(text);
    }
    ...
    if let Some(text) = raw
        .get("output")
        .and_then(|value| value.as_array())
        .and_then(|items| join_text_blocks(items.iter().filter_map(collect_text)))
    {
        return Some(text);
    }

    raw.get("output").and_then(collect_text)
}
...
fn extract_from_top_level_message(raw: &Value) -> Option<String> {
    ...
    raw.get("content").and_then(collect_text)
}
...
fn extract_from_typed(typed: &ChatCompletionResponse) -> Option<String> {
    typed
        ...
        .and_then(normalize_output_text)
}
...
pub(crate) fn preview_from_stream_text(text: String, max_len: usize) -> Option<String> {
    normalize_output_text(text).map(|text| truncate(text, max_len))
}

pub(crate) fn stream_chunk_preview_fragment(raw: &Value) -> Option<String> {
    if let Some(event_type) = raw.get("type").and_then(|value| value.as_str())
...
mod tests {
    ...
    #[test]
    fn extracts_markdown_from_responses_output_without_trimming_structure() {
        let dual = dual_from_raw(json!({
            "id": "resp-markdown",
            "object": "response",
            "created": 0,
    ...
    #[test]
    fn extracts_stream_chunk_preview_fragment_from_responses_delta_event() {
        let chunk = json!({
